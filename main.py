...
def render_frame(i):
    ...
    ax.set(xlim=limits['x'], ylim=limits['y'], zlim=limits['z'], xlabel='$x$', ylabel='$y$', zlabel='$r$')
...
if __name__ == '__main__':
    ...
    x, y = cp.meshgrid(
        cp.linspace(limits['x'][0], limits['x'][1], surf_res['x']),
        cp.linspace(limits['y'][0], limits['y'][1], surf_res['y']),
        indexing='ij'
    )
    dx = x[1, 0] - x[0, 0]
    dy = y[0, 1] - y[0, 0]
    ...
    kx, ky = cp.meshgrid(
        cp.fft.fftfreq(x.shape[0], d=dx) * 2 * cp.pi,
        cp.fft.fftfreq(y.shape[1], d=dy) * 2 * cp.pi,
        indexing='ij'
    )
    k2 = kx ** 2 + ky ** 2
    ...
    if render_wavefunction:
        limits['z'] = [0, float(find_limit(mod, .95))]
    else:
